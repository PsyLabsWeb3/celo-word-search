...
export const useLegacyContractStats = (contractAddress: string) => {
  const [stats, setStats] = useState<LegacyContractStats>({
    completedEvents: [],
    prizeEvents: [],
    loading: true,
    error: null,
  ...
  useEffect(() => {
    const fetchContractStats = async () => {
      ...
      try {
        ...
        console.error(`Error fetching data from legacy contract ${contractAddress}:`, error);
        setStats({
          completedEvents: [],
          prizeEvents: [],
          loading: false,
          error: `Error fetching data from legacy contract: ${error instanceof Error ? error.message : 'Unknown error'}`,
        });
      }
    };

// Test script for local Hardhat node flow
// 2 users create crosswords, add CELO rewards, add 3 winners each
// 6 users solve crosswords, verify balance changes for winners

const hre = require("hardhat");

async function testLocalFlow() {
  console.log("🧪 Starting local Hardhat flow test...\n");
  ...
  // Grant admin role to the CrosswordBoard contract on CrosswordPrizes so it can create crosswords
  await CrosswordPrizes.write.grantRole([adminRole, CrosswordBoard.address], { account: admin.account });
  console.log("✅ Granted admin role to CrosswordBoard on CrosswordPrizes");
  ...
  const crosswordData2 = JSON.stringify({
    ...
    clues: {
      across: [
        { number: 1, clue: "Difficult" },
        { number: 2, clue: "Blockchain platform" },
        { number: 3, clue: "To provide" },
        { number: 4, clue: "Computer science field" }
      ],
      down: [
        { number: 1, clue: "Beginning" },
        { number: 2, clue: "Past tense of 'is'" },
        { number: 3, clue: "Opposite of 'soft'" },
        { number: 5, clue: "Technology company" }
      ]
    }
  });
  ...
  // Create first crossword with native CELO prize pool (10 CELO total, 3 winners)
  ...
  // Create second crossword with native CELO prize pool (15 CELO total, 3 winners)
  ...
  // Simulate first 3 solvers completing the first crossword
    ...
    try {
      ...
      const messageHash = keccak256(
        encodePacked(
          ...
          [CrosswordBoard.address, currentId, duration, CrosswordCore.address]
        )
      );

      // Sign the message using the admin account (the signer)
      // The contract expects a signature on the Ethereum-prefixed hash of the messageHash
  ...
  // Simulate next 3 solvers completing the second crossword
  ...
  // Check winner information for both crosswords
  ...
  // Check final balances and verify prize distribution
  console.log("\n📊 Checking final balances and verifying prize distribution...");
  const finalBalances = {};
  for (let i = 0; i < 6; i++) {
    const solver = solvers[i];
    finalBalances[solver.account.address] = await publicClient.getBalance({ address: solver.account.address });
    const initial = initialBalances[solver.account.address];
    const final = finalBalances[solver.account.address];
    const difference = final - initial;
    
    console.log(`  - Solver${i+1} balance change: ${(Number(difference) / 1e18).toFixed(6)} CELO`);
    
    // Calculate expected prize based on which crossword they completed and their rank
    let expectedPrize = 0n;
    if (i < 3) { // Completed crossword 1
      if (i === 0) expectedPrize = (prizePool1 * 5000n) / 10000n; // 50% for 1st place
      else if (i === 1) expectedPrize = (prizePool1 * 3000n) / 10000n; // 30% for 2nd place
      else if (i === 2) expectedPrize = (prizePool1 * 2000n) / 10000n; // 20% for 3rd place
    } else { // Completed crossword 2
      if (i === 3) expectedPrize = (prizePool2 * 4000n) / 10000n; // 40% for 1st place
      else if (i === 4) expectedPrize = (prizePool2 * 3500n) / 10000n; // 35% for 2nd place
      else if (i === 5) expectedPrize = (prizePool2 * 2500n) / 10000n; // 25% for 3rd place
    }
    
    if (expectedPrize > 0) {
      console.log(`    - Expected prize: ${(Number(expectedPrize) / 1e18).toFixed(6)} CELO`);
      if (difference >= expectedPrize) {
        console.log(`    - ✅ Prize received (may include gas refunds)`);
      } else {
        console.log(`    - ⚠️  Prize may still be claimable or difference is due to gas costs`);
      }
    } else {
      console.log(`    - No prize expected for this position`);
    }
  }
  ...
  console.log("   - Winners received prizes automatically upon completion");

...
async function testFullFlow() {
  ...
  // Set CrosswordBoard as the funding contract for CrosswordPrizes
  await CrosswordBoard.write.setFundingContractForPrizes([CrosswordBoard.address], { account: deployer.account });
  console.log("✅ Set CrosswordBoard as funding contract for CrosswordPrizes");
  ...
  const completeCrossword1 = async (solver, duration, username) => {
    try {
      ...
      // where msg.sender should be the user's address, but since we're calling through CrosswordBoard,
      // the contract sees CrosswordBoard as msg.sender, so we sign with that address
      const messageData = encodePacked(
      ...
      // So we need to sign the messageHash directly (viem.signMessage does the Ethereum prefix automatically)
  ...
  // 6. Check final balances and verify prize distribution

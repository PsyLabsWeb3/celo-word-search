...
async function main() {
  console.log("🔍 Verifying deployed contracts on Celo Sepolia...\n");

  // Get the provider
  const provider = hre.ethers.provider;

  // Check the old contract that might be deployed
  const oldContractAddress = "0x6e15f23e7f410E250BD221cdB2FB43840354C908";
  console.log(`Checking old CrosswordBoard contract at ${oldContractAddress}...`);
  
  try {
    const code = await provider.getCode(oldContractAddress);
    const hasCode = code && code !== "0x";
    console.log(`  ✅ Old contract exists: ${hasCode}`);
    if (hasCode) {
  ...
  // Now let's deploy the modular contracts if they don't exist
  console.log("\n🚀 Deploying modular contracts to Celo Sepolia...\n");

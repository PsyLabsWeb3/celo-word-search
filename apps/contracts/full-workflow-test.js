...
async function main() {
  ...
  // Simulate players completing the crossword
  console.log("\n🏃‍♂️ Simulating crossword completions for 10 players...");
  
  // For signature testing, we need to create valid signatures
  // In a real scenario, the backend service would generate these
  const durations = [];
  const signatures = [];

  // Generate completion times for players
  for (let i = 0; i < 10; i++) {
    durations.push(300000 + i * 10000); // 5-6 minutes with small differences
  }

  // For this test, let's check if the contracts accept completions properly
  // by testing a few players (signature generation is complex in this environment)
  console.log("Testing that the contracts accept completions properly...");
  ...
  // Verify user completion tracking works
  const completionsBefore = await crosswordCore.getCrosswordCompletions(crosswordId);
  console.log(`📊 Completions before: ${completionsBefore.length}`);
  ...
  console.log(`✅ CrosswordPrizes: Max winners configured to ${maxWinners}`);

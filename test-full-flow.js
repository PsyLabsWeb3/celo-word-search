...
const CONTRACT_ADDRESS = "0x262F9Acedc7def0ee4eA551293203FF07a249bF0"; // Our newly deployed contract
...
async function testFullFlow() {
  ...
  const abiPath = './apps/contracts/artifacts/contracts/CrosswordBoard.sol/CrosswordBoard.json';
  ...
  try {
    ...
    const receipt1 = await walletClient.waitForTransactionReceipt({ hash: hash1 });
    ...
    const prizePool = parseEther('1'); // 1 CELO prize pool
    ...
    // Wait for transaction to be mined
    const receipt2 = await walletClient.waitForTransactionReceipt({ hash: hash2 });
    ...
    // Wait for transaction to be mined
    const receipt3 = await walletClient.waitForTransactionReceipt({ hash: hash3 });
    ...
    try {
      ...
      // Wait for transaction to be mined
      const receipt4 = await walletClient.waitForTransactionReceipt({ hash: hash4 });
